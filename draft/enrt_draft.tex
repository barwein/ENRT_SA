...
Then, in the second stage, each recruited ego recruits a subset of its neighbors in the population network as alters (non-index).
Formally, the recruitment process is as follows:
\begin{enumerate}
    \item Sample egos from the population.
    Denote the set of recruited egos by $\cR_e \subset V$.
    ...
    We assume that each alter is linked to a single ego, resulting in $n_e$ disjoint ego-networks.
...
This exposure mapping specification is common in the ENRT literature \citep{Buchanan2018,Chao2023,Fang2023}.
...
We explicitly separate the causal estimands for egos and alters, as recommended in the literature \citep{Crawford2019}.
In that we avoid conflating the two groups,
as the recruitment process may induce different selection mechanisms for egos and alters, 
leading to incomparable distributions of potential outcomes between the two groups.
\\
\\


Note that conditional on recruitment $\cR$, 
not all potential outcomes are necessarily well defined. 
We observe a star ego-network $\widetilde{\bA}$, where each ego is connected to its alters, and alters and egos are not connected to each other. 
If in the population network $\bA$, each recruited ego is not connected to any other recruited ego, then $Y_i(z,1),\; z\in \{0,1\}$
is not well defined for the egos, as no ego can be exposed to the treatment of another ego.
Furthermore, since alters cannot be assigned directly to treatment, $Y_i(1,f), \; f\in \{0,1\}$ is not well defined for alters. 


Moving on to the positivity of treatment and exposures.
For alters, we have $\Pr(Z_i=0, F_i=f \mid i \in\cR_a) >0, \; f\in \{0,1\}$ by Assumptions~\ref{ass:bernoulli_design} and the recruitment process. 
By Assumption~\ref{ass:bernoulli_design}, $\Pr(Z_i=z, F_i=f \mid i \in\cR_e) = \Pr(Z_i=z \mid i \in\cR_e)\Pr(F_i=f \mid i \in\cR_e)$, and $\Pr(Z_i=z \mid i \in\cR_e)\in (0,1)$. Finally, let,
\begin{equation}
    \label{eq:ego_expos_prob}
    \pi^e_i = \Pr(F_i=1 \mid i \in\cR_e),
\end{equation}
denote the probability that ego is exposed to at least one treated neighbor in the population network $\bA$.
\begin{assumption}[Positivity of egos exposures]
    \label{ass:ego_postivity}
    $\pi^e_i \in (0,1) \; \forall i \in \cR_e$.
\end{assumption}
Specifically, Assumption~\ref{ass:ego_postivity} states that each ego is connected to at least one, but not all, other ego. The potential outcomes of egos $Y_i(z, f), i\in\cR_e$ are well defined for $f \in \{0,1\}$ only if $\pi^e_i \in (0,1)$, which is the crux of our proposed argument. As the population network $\bA$ is only partially observed, it cannot be verified from the observed data, however, it is required for proper interpretation.
Note that Assumption~\ref{ass:ego_postivity} does not hold if egos recruit all their neighbors in the population network as alters, rather than a subset. 
...
computed on the \emph{population network}.
The observed exposures are $\widetilde{F}_i = F(\bZ_{-i}, \widetilde{\bA})$,
which may differ from the true exposures $F_i$. 
That is, the observed network $\widetilde{\bA}$ might not capture all the connections present in the population network $\bA$ 
...
if only ego 1 is treated, alter 6 will have observed exposure $\widetilde{F}_i=0$ since its ego (node 2) is not treated,
but will have true exposure of $F_i=1$ due to its link to ego 1 in the population network. 
Similarly, ego 2 have an observed exposure of $\widetilde{F}_i=0$ while $F_i=1$ in the population network
...
then for some units the observed exposures $\widetilde{F}_i$ may not be equal to the true exposures $F_i$ under all possible treatment assignments $\mathcal{Z}$.
...
\end{proposition}
Only when an alter is connected only to its own ego, $\pi_i^a=p_z$, and \eqref{eq:IE_naive_HT} will be design-unbiased estimator of \eqref{eq:indirect_effect}.
...
Similarly to alters, the magnitude of the bias will depend on the number of other egos each ego is connected to in the population network.
 
\section{Sensitivity analysis for contamination between ego-networks}
\label{sec:sa}

Proposition~\ref{prop:bias_naive_ht} shows that the HT estimators based on the observed data are biased whenever there is contamination between the ego-networks.
By the structure of the egocentric sampling and the exposure mapping (Assumption~\ref{ass:expos_map}), alters with observed exposure $\widetilde{F}_i=1$ will be correctly classified. However, those with $\widetilde{F}_i=1$ can have a true exposure of $F_i=1$ or $F_i=0$, depending on the structure of the population network $\bA$. That is, the observed outcomes $Y_i$ for alters with $\widetilde{F}_i=0$ can be generated from $Y_i(0,1)$ or $Y_i(0,0)$. 

On the other hand, all egos have observed exposures of $\widetilde{F}_i=0$, as they are linked only to alters in the ego-network $\widetilde{\bA}$. Thus, the observed outcomes of egos that are assigned to treatment $Z_i=z$ can be generated from $Y_i(z,1)$ or $Y_i(z,0)$. Thus, the observed outcomes of egos are possibly linked to four different potential outcomes, compared to only two among the alters.

We develop a sensitivity framework to estimate the direct and indirect effects, while accounting for possible exposure misspecification due to the egocentric recruitment process. For indirect effects \eqref{eq:indirect_effect}, we derive identification formulas that represent the estimands as a function of observable quantities and one sensitivity parameter. The parameter is related to the probability of edges between recruited egos and alters in $\bA$ and is further described in Section~\ref{subsec:rho_param}.
For direct effects \eqref{eq:directed_effect}, we derive an identification formula for the direct effects using an additional sensitivity parameter.
All proposed frameworks are designed to be simple post-analysis procedures that can be used to assess the validity of the causal claims under study against the threat of contamination between ego-networks.  

\bwnote{TODO: write some clarification that the SA or bias analysis for IE will depend on $\pi_i^a$, which can be simplified to specifying the number or probability of missing ego-alter edges (edges of alters to egos in different ego-networks). Similar thing to the egos about $\pi_i^e$ but with the additional sensitivity parameter $\kappa$.}
...
\bwnote{TODO: split this paragraph. First part about bias magnitude move to previous Prop, and the part about attenuation towards the null keep here.}
Moreover, the magnitude of the bias depends on the gap between $\pi^a_i$ and $p_z$, which in turn depends on the number of possible edges between alters and other recruited egos.
By construction, $\widetilde{F}_i \leq F_i$ with probability one, as the neighbor sets in the observed network $\widetilde{\bA}$ are a subset of the sets in the population network $\bA$, and therefore $p_z \leq \pi^a_i $. Consequently, if each alter has the same exposure probabilities $\pi^a_i = \pi^a$ or that in the data each alter improves when exposed $\EX \big[Y_i \mid Z_i=0, \widetilde{F}_i=1, i \in\cR_a \big]\geq \EX \big[Y_i \mid Z_i=0, \widetilde{F}_i=0, i \in\cR_a \big]$, or vice versa, then the naive estimators \eqref{eq:indirect_effect_estimator} are biased towards the null of no indirect effect for alters (Appendix~\ref{apdx_sec:proof}).


\bwnote{Expand on this point.} Can modify the estimator \eqref{eq:IE_corrected} to include outcome model as well. Let $m^a_i(f) = \EX \left[Y_i \mid \widetilde{F}_i=f, \bX_i, i \in \cR_a \right]$
be the conditional expectation among alters with observed exposure $\widetilde{F}_i = f$ and covariates $\bX_i$. In practice, we estimate it using the observed outcomes and exposures using some sort of cross-fitting such that $m^a_i(f)$ are independent of the design (of $\bZ$). We can use an augmented bias-corrected estimator
\begin{equation}
    ...
    \right],
\end{equation}
which is still approximately design-unbiased as \eqref{eq:IE_corrected}, but can have lower variance.

\bwnote{Variance estimation... in the appendix...}

\subsection{Direct effects}
\label{subsec:de}

As the observed outcomes of egos are possibly generated from four different potential outcomes, compared to only two for the alters, bias correction for the direct effect estimator requires an additional structural assumption. Specifically, we require that the unit-level direct effects among exposed ($F_i=1$) and non-exposed ($F_i=0$) egos are proportional
\begin{assumption}
...
\end{assumption}
The interpretation of the additional sensitivity parameter $\kappa$ in Assumption~\ref{ass:egos_kappa} is related to the interaction term between treatment and exposure in the potential outcome model of the egos. Consider the saturated potential outcomes model
\begin{equation*}
    Y_i(z,f) = \beta_{0i} + \beta_{1i}z + \beta_{2i}f + \beta_{3i}zf,
\end{equation*}
then Assumption~\ref{ass:egos_kappa} is equivalent to $\beta_{3i} = (\kappa-1)\beta_{1i}$. That is, the interaction term $\beta_{3i}$ is proportional to the direct effect term $\beta_{1i}$ for all egos.
Reasoning about the range of plausible $\kappa$ values depends on the mechanism under study. For example, in vaccine efficacy studies, if treatment represents vaccines, it is reasonable to assume that direct effects among egos with only non-vaccinated friends will be larger than those among egos exposed to vaccinated units, and therefore $\kappa < 1$.
