...
The second estimand is the sample average direct effect of treatment on the egos direct effect of 
    treatment on the egos
\begin{equation}
...
\begin{exmp}[Homogeneous probabilities]
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            \pi_i^a &= p_z + (1-p_z)\left[1- (1-p_z\rho^a)^{n_a-1}\right].
        \end{aligned}
    \end{equation*}
...
\begin{exmp}[Homogeneous number of edges]
    ...
    is connected to $k>0$ additional egos on average.
    This can be represented by selecting $m^a = k (n_e-1)$.     
\end{exmp}
...
As the the true values of such parameters are unknown and can not be directly estimated from the data,
 ...
 and compute the bias-corrected estimates for each combinatio
...
So we can derive the sampling distribution of $\widehat{\theta}_{adj}(\bD, \rho^{(m)})$
(e.g., approximate with normal distribution),
and then for each draw $\rho^{(m)}$ from $p(\rho)$, estimate the adjusted estimator
