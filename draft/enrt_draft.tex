...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/population_and_ego_network.png
    }
...
We report here the results where contamination is generated using   
heterogeneous edge probabilities as in Example~\ref{exmp:pi_hetero_number}
with Euclidean distance and $\gamma^e=\gamma^a=1$.
Tables~\ref{tab:ie_results_main} and \ref{tab:de_results_main} summarize the results
for the indirect and direct effects, respectively, using both analytic and bootstrap variance estimation.
We use the augmented estimators \eqref{eq:IE_corrected_aipw} and \eqref{eq:DE_corrected_aipw}
...
The Naive specification corresponds to the uncorrected estimators \eqref{eq:IE_naive_HT}-\eqref{eq:DE_naive_HT}.
As the level of contamination ($m_e, m_a$) increase,
...
Additional results, including homogeneous contamination settings and non-augmented estimators,
...
\caption{
Simulation results for direct effects under heterogeneous contamination 
with augmented estimators.
Error is the empirical bias, Coverage is the empirical coverage of the 95\% confidence intervals,
and SD/SE is the ratio of standard deviation of the estimates to the mean standard errors.
Specification ``Heterogeneous" uses the correct edge probabilities,
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/hptn/sa_ie_plot_augmented.png}
    \caption{SA IE Augmented}
    \label{fig:sa_ie_aug}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/hptn/pba_de_augmented.png}
    \caption{PBA DE Augmented}
