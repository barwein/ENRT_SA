...
On the other hand, in the HPTN-037 study \citep{Latkin2009}, 
    ...
    the direct effect is likely to be larger for egos exposed to a treated neighbor, leading to $\kappa > 1$.
...
Moreover, under Assumption~\ref{ass:egos_kappa}, the naive estimator $\widehat{IE}$ can be shown to be
 attenutated \emph{away} from the null if $\kappa >1$, and attenuated \emph{towards} the null if
...
Thus, in general, the naive estimator is attenuated towards the null if $\kappa <1$ (Appendix~\ref{apdx_sec:proof}).
...
By the exposure mapping specification (Assumption~\ref{ass:expos_map}),
...
and ego--ego edges $\left\{A_{ij}: i \neq j \in \cR_e\right\}$ 
    are the relevant missing edges from $\widetilde{\bA}$.
...
\end{equation}
Alter $i\in\cR_a$ is exposed $F_i=1$ if its ego $e(i)$ or at least one of the other recruited 
...
\begin{exmp}[Homogeneous number of edges]
    ...
    This can be represented by settings $m^a = k (n_e-1)$.     
\end{exmp}
...
\label{exmp:pi_hetero_number}
    ...
    pair-specific heterogeneous probabilities while fixing only the expected number of missing edges.
...
\bwnote{TODO: continue here. Write the general overview. Then write the steps for grid SA and PBA.
Mention variance estimation in both. Describe interpretation of results.}
...
\begin{enumerate}[label = (\roman*)]
    ...
    $\mu^a_i(f), \mu^e_i(z)$. Use cross-fitting via $k$-fold split (deafult can be two-fold split, namely, split data into two folds (e.g., split into two folds of ego-networks), estimate outcome model in one fold and predict $\mu^e_i, \mu^a_i$ on the data from the second one, and vice versa). If not using the augmented estimators, we can set $\mu^e_i = \mu^a_i = 0$.
...
We assume that the outcome models $\mu^a_i(f)$ are estimated through cross-fitting (\bwnote{TODO: add ref}) and therefore assumed to be independent of $\bZ$. We ignore the sampling uncertainty associated with estimating the outcome models. 
