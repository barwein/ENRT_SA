...
#' Calculate augmented IE_RD and its cluster-robust variance for a single pi_vec
#' #'
...
#' Calculate bias-corrected DE_RD for a single (pi, kappa) combination
#'
...
#' Estimate bias-corrected DE for a single pi_param and a grid of kappa values
#'
#' @param Y_e Vector of observed ego outcomes.
#' @param Z_e Vector of observed ego treatments.
#' @param mu_10 Vector of predictions E[Y|Z=1, X].
#' @param mu_00 Vector of predictions E[Y|Z=0, X].
#' @param pi_vec Vector of pi_i^e values.
...
#' @param mu_10 Vector of predictions E[Y|Z=1, X] (n_e).
#' @param mu_00 Vector of predictions E[Y|Z=0, X] (n_e).
...
de_grid_multi_pi_kappa <- function(Y_e,
  ...
  if(is.null(mu_e_1) || is.null(mu_e_0)) {
    warning("mu_10 or mu_00 is NULL. Setting both to zero vectors.")
    mu_e_1 <- rep(0, n_e)
    mu_e_0 <- rep(0, n_e)
  }
  else{
    if (n_e != length(mu_e_1) ||
        n_e != length(mu_e_0)) {
      stop("Y_e, mu_10, and mu_00 must have the same length (n_e).")
    }
  }
